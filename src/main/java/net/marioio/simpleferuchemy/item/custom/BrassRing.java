...
public class BrassRing extends FeruchemicRing {
    private static MobEffect inEffect = MobEffects.CONFUSION;
    private static MobEffect outEffect = MobEffects.FIRE_RESISTANCE;
    protected static final String[] modes = {"text.simpleferuchemy.storing_a_lot_masc", "text.simpleferuchemy.storing", "text.simpleferuchemy.idle", "text.simpleferuchemy.extracting", "text.simpleferuchemy.extracting_a_lot_masc"};

    public BrassRing(Properties pProperties) {
        super(pProperties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ...
        if (super.use(pLevel, pPlayer, pUsedHand).getResult() == InteractionResult.SUCCESS  && !pLevel.isClientSide()){
            ...
            if (current_mode != 0) {
                msg = msg.append(Component.literal(" ").append(Component.translatable("text.simpleferuchemy.heat")));
            }

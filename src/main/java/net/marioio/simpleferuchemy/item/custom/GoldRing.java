...
public class GoldRing extends FeruchemicRing {
    private static MobEffect inEffect = MobEffects.POISON;
    private static MobEffect outEffect = MobEffects.REGENERATION;

    public GoldRing(Properties pProperties) {
        super(pProperties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ...
        if (super.use(pLevel, pPlayer, pUsedHand).getResult() == InteractionResult.SUCCESS  && !pLevel.isClientSide()){
            ...
            if (current_mode != 0) {
                msg = msg.append(Component.literal(" ").append(Component.translatable("text.simpleferuchemy.health")));
            }

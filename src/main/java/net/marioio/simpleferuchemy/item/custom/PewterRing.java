...
public class PewterRing extends FeruchemicRing {
    private static MobEffect inEffect = MobEffects.WEAKNESS;
    private static MobEffect outEffect = MobEffects.DAMAGE_BOOST;

    public PewterRing(Properties pProperties) {
        super(pProperties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ...
        if (super.use(pLevel, pPlayer, pUsedHand).getResult() == InteractionResult.SUCCESS  && !pLevel.isClientSide()){
            ...
            if (current_mode != 0) {
                msg = msg.append(Component.literal(" ").append(Component.translatable("text.simpleferuchemy.strength")));
            }
